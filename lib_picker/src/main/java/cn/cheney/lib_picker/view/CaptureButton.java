...
import static cn.cheney.lib_picker.XPickerRequestKt.MIXED;
import static cn.cheney.lib_picker.XPickerRequestKt.ONLY_CAPTURE;
import static cn.cheney.lib_picker.XPickerRequestKt.ONLY_RECORDER;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = ONLY_CAPTURE;  //初始化按钮为可录制可拍照
        ...
        try {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    //判断按钮状态是否为可录制状态
                    if ((button_state.equals(ONLY_RECORDER)
                            || button_state.equals(MIXED)))
                        postDelayed(longPressRunnable, 500);    //同时延长500启动长按后处理的逻辑Runnable
                    break;
                case MotionEvent.ACTION_MOVE:
                    if (captureLisenter != null
                            && state == STATE_RECORDERING
                            && (button_state.equals(ONLY_RECORDER) || button_state.equals(MIXED))) {
    ...
    private void handlerUnpressByState() {
        try {
            ...
            switch (state) {
                //当前是点击按下
                case STATE_PRESS:
                    if (captureLisenter != null && (button_state.equals(ONLY_CAPTURE)
                            || button_state.equals(MIXED))) {
